...
2. **Before any relaunch on either machine**, copy `GameLogFull.txt` from *both* machines (each launch
   rotates `GameLogFull.txt` to `GameLogFullPrev.txt` and replaces the previous `Prev`). One-sided
...
- Transpiler counts: `[FogGate]` six site lines; `[TimeScaleFix]` x2; `[IdleRng]` x4 (counts 5/1/1/4);
  `[ProjectileFix]` swap line.
- No `PATTERN NOT FOUND` lines (each means that one site reverted to vanilla).

Runtime activation lines appear on first qualifying events, not at boot: `[ChargeFix] Active`,
...
### Charge and parry same-tile case (E17, required validation for C22)

1. Both machines on version 0.8.30 or later. In turn-based combat, **hover a charge path that extends past an enemy**,
...
### Trap and pause containment (E14, required validation for C20)
...
### Departure handling (E20, regression check)
...
full DesyncWatch episode (`POTENTIAL`, bucket attribution, `SERIOUS`, and dumps). Note: cheats are
