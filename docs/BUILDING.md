...
Verified against the v0.8.32 baseline (`RELEASE-0.8.32.md` holds all hashes referenced here).
...
| Reference assemblies | The template's `Assets/RogueTraderAssemblies/`. **Do not expect the `Code.dll` hash in `RELEASE-0.8.32.md` to match yours.** The build described here used a locally re-serialized `Code.dll` (`33002BB3...`, 14,228,480 B), replacing the template copy (`8505AA6E...`, 14,228,992 B, retained beside it). A 1.6.1.514 game installation contains another variant (`3F94862B...`). All 57 documented target rows (55 patch targets and 2 reflection dependencies) resolve against all three variants. Build against the assembly supplied by your template and confirm it with `tools/check-harmony-targets.py`. `0Harmony.dll` SHA-256: `9611251F080E4855CD9D5FA54B5E56034DF25B70B374C49BA09550B08A1BF875` |
...
These are separate operations. Verify each one before distribution.
...
2. **Package:** run the template's Owlcat modification build (Unity menu:
   **Modifications > Build**, then select `MultiplayerStability`). Output:
...
`tools/verify-package.ps1` automates this check (see `TESTING.md`). Do not distribute a build based
only on a successful Unity compile.
...
Runtime activation lines appear on first qualifying events (full list in `REPRODUCING.md`).
...
- Validation requirement: a prevention fix is *field validated* only after a post-fix two-sided capture on the
...
- After a game update: run `tools/check-harmony-targets.py` against the updated assemblies
  (standard set in the tool header), then complete the runtime verification above. Re-diff
  `DeterministicSleep`'s replicated vanilla verdict against
