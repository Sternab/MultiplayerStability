...
the entry point). Statuses use a strict vocabulary:
**Field validated** (a post-fix two-sided capture shows the class absent), **Mechanism confirmed;
post-fix validation pending** (the mechanism is established from captures or source, but the fix has not
been verified in a post-fix capture), **Diagnostic only**, **Withdrawn or superseded**, and
**Infrastructure**.

Compatibility categories: **subset-safe** (any subset of machines), **negotiated** (self-gates on
every-peer-modded), and **exact-parity** (simulation-changing; identical build required on every machine
until the 0.9 latch ships).

Component source categories: **ENGINE** (the relevant path is in engine code; the mod applies a downstream
workaround), **DIAG** (diagnostic tooling), and **INFRA** (mod-only infrastructure). Entries for
confirmed issues end with a **source note** that identifies the relevant engine path. Entries
for open issues state the unresolved question instead.
...
- **Observed behavior:** save-transfer window refills only on acks; acks are sent once per rendered frame,
  and transfers run during low-FPS loading screens -> ~0.22 MB/s.
- **Mod behavior:** 8 ms `SendAcksOnly` timer pump during any transfer (always); 96 KB x 4
...
- **Source note:** the transfer ceiling comes from the frame-clocked ack pump and the default
  chunk/window constants, not from available bandwidth.

## C02: Steam P2P save transfer (`SteamP2P.cs` and `SteamSaveTransfer.cs`, INFRA)
...
- **Targets:** `DataTransporter.SendSave` (prefix), `MessageNetManager.OnMessage` (prefix; claims
  ...
  access.
- **Observed behavior:** all save bytes traverse the Photon relay at ~230 KB/s per peer.
- **Mod behavior:** bulk bytes over `SteamNetworkingMessages` (ICE direct / SDR relay), adaptive
  ...
  per-peer sends. ~8x measured speedup; 10/10 recent field transfers delivered (`fed to game=True`).
...
- **Source note:** this side channel exists only because the relay path is rate-capped; it adds
...
- **Observed behavior:** render-loop weather VFX drain the hashed `Weather` stream once+ per frame;
...
- **Source note:** the engine already maintains non-serialized VFX streams (e.g. `Visuals.Fx`);
...
- **Observed behavior:** aim-bone pick draws hashed `Projectiles` **only when** the target view has a
...
- **Mod behavior:** deterministic first-locator pick; the unconditional draws in the same method stay
  hashed. A zero-swap result reports `PATTERN NOT FOUND`.
...
- **Status:** Field validated (the class disappeared from captures after the fix; later geometry hole
  addressed by C15).
- **Source note:** the draw itself is cosmetic (bone choice); the desync comes from its
...
- **Observed behavior:** one reused `NetLockPointId` with no sequence number -> a fast client's
...
- **Source note:** the race exists because barrier announcements carry no sequence identity:
...
- **Observed behavior family:** the awake-unit census is camera/fog-driven (client-local) while gating
...
- **Source note:** the defect is simulation scheduling being coupled to render culling; the mod's
...
- **Targets (transpilers, getter-swap):** `AreaEffectEntity.ShouldUnitBeInside(BaseUnitEntity)`,
  ...
  keeping the synced `IsInGame` term), `UnitMovementAgentBase.TickMovement` (fog-gated 8x speed +
...
- **Observed behavior:** mechanics decisions read the client-local fog/render flags; hashed positions,
...
- **Source note:** the six sites are the mechanics-side reads of `IsInFogOfWar`/
...
- **Observed behavior:** dash-through delivery polls the caster's live mid-dash **view** position per
...
- **Source note:** the per-frame delivery poll is the one place this ability family reads a live
...
- **Observed behavior:** client-local preview units (inventory dolls, level-up plans) stay subscribed to
...
- **Source note:** the root condition is preview units subscribing to gameplay events and sharing
...
- **Behavior:** logs any draw of a serializable/hashed stream outside a deterministic simulation tick,
  with a classified call site. It can identify a candidate source from one machine before a desync occurs.
  Log-only by design (auto-diverting a false positive would manufacture a desync).
- **Category:** subset-safe; works solo.
- **Validated behavior:** reported the Solomorne dialogue draws (C16 Guard C) before the fork.
...
- **Observed behavior:** a preview unit's global rulebook handlers fire during real combat and fork the
...
- **Source note:** same root condition as C10: a preview unit's handlers living on the global
  rulebook bus.
...
- **Targets (transpilers):** `TurnController.SetTime` (fog-read swap), `UnpauseController.Tick`
  (0.6f constant swap).
- **Observed behavior:** two client-local writers of `PlayerTimeScale`, which scales the 50 ms step into
  hashed `GameTime`: a fog-gated 16x AI-turn fast-forward (fog is client-local) and a local slow-mo
  input hold. One tick of disagreement is a permanent `player`-bucket fork.
- **Mod behavior:** in MP the AI-turn scale is always 1x (a camera-frustum substitution was withdrawn:
  the frustum test culls against local renderer bounds and is a proxy, not deterministic), and the
  slow-mo constant becomes 1x.
...
- **Source note:** one time-scale factor is already a synchronized input (`CameraFollowTimeScale`
...
- **Observed behavior:** results come back in physics-broadphase order (collider creation/toggle history:
...
- **Source note:** the engine's own sibling `FindUnitsInShape` already orders its results with
...
- **Observed behavior:** projectile mechanical geometry reads live view-bone transforms: the stored
...
- **Status:** Mechanism confirmed; post-fix validation pending.
- **Source note:** comparable lockstep engines separate visual bones (rendering) from
...
- **Observed behavior:** dialogue UI refresh paths draw the hashed `DialogSystem` stream at view time:
...
- **Withdrawn:** a timing-based gate (`IsSimulationTick`) was disproved in field captures; a global
  deterministic `CueSelection` replacement changed real narrative selection. Both documented
...
- **Source note:** the defect shape is UI preview/inspection paths sharing hashed RNG streams
...
- **Observed behavior:** `AnimationManager.StatefulRandom` maps idle-variety draws to the **hashed**
...
- **Source note:** `AnimationIdle` already exists as the engine's designated non-hashed idle
  stream; the idle call graph maps to hashed `Animation3` instead.
...
- **Observed behavior:** on player-leave the engine raises a role event per controlled entity (~1,500);
...
- **Source note:** `HandleRoleSet` receives an `entityId` parameter it does not use, and the log
...
- **Open issue (mechanism confirmed, predicate unproven):** the combat-exit inclemency choice reads
...
## C20: TrapPauseDiag and containment (`TrapPauseDiag.cs`, ENGINE containment and DIAG)
...
- **Observed behavior:** trap detection auto-pauses; while paused, `AbstractUnitCommand.OnRun` calls
  ...
  members. Longstanding defect (present in early test logs). Field counts: 72-vs-10 NREs,
...
- **Status:** containment Mechanism confirmed; post-fix validation pending (no post-fix trap capture
  yet). Diagnostic Field validated as instrumentation (27 matching cross-peer records in a clean
  session).
- **Source note:** the defect window is view/IK work running inside command `OnRun` between the
...
- **Observed behavior:** the client-local augmentation screen picks a bark with `UnityEngine.Random` and
  raises the banter event; the handler adds it to `Player.PlayedBanters`, which is in the
  synchronized player hash. The write occurs only on the peer that opened the screen.
- **Mod behavior:** caller-scoped containment: the handler skips only while the augmentation VM
  ...
  verified as symmetric) are untouched. Cost: no augmentation-screen bark in co-op.
- **Gate/category:** MP-gated; exact-parity.
- **Status:** Mechanism confirmed; post-fix validation pending.
- **Source note:** `PlayedBanters` is in the synchronized player hash while this write originates
  from a client-local screen: a one-sided write by construction.

## C22: ChargePathDiag and fix (`ChargePathDiag.cs`, ENGINE fix and DIAG)
...
- **Observed behavior:** the partial charge-path cache lookup matches caster + origin + ignoreBlockers
  only, with no destination key and **no target entity**, then cuts a cached path at the destination's
...
- **Mod behavior (MP-only):** partial-cache reuse disabled (prefix returns null); exact target-checked
  hits stay cached; unmatched paths recompute; solo untouched. Verification requires three signals: no
  `Partial_Patch` init error, the one-time `[ChargeFix] Active` line, and no `partial-cache` source
  lines. Silence alone is inconclusive because the prefix and validator share one patch class.
- **Gate/category:** MP-gated; exact-parity.
...
- **Source note:** the Dark Heresy build of this engine no longer contains the partial lookup and
  keys full-cache hits on target identity. This patch approximates that behavior without changing
...
- **Open issue (mechanism confirmed, origin unproven):** the component accumulates a fractional
...
- **Status:** Diagnostic only.
- **Source note:** the accumulator is gameplay-relevant state omitted from the component hash;
