...
*How multiplayer actually works, from the decompiled game source.
Compiled 2026-07-02 from an 8-subsystem deep-read (full per-subsystem reports with ~120 file:line cites:
per-subsystem reports retained privately by the author; not published).*

## The model: deterministic lockstep over Photon

Only **player commands** cross the wire. Each client runs the full simulation independently and must produce
bit-identical state; correctness is checked by comparing state hashes. Any non-determinism = desync.
...
- **Escape hatches (by design, and dangerous):** `GameCommand.IsSynchronized` defaults **false** — unsynced
...
- Pause is a synced game mode (packets keep flowing while paused). Turn-based combat is not a special tick
  mode — `TurnController` is just another Simulation-tick controller.
...
- **Every entity/fact UniqueId comes from the hashed GlobalUuid stream** (`Uuid.cs:54-64`, `EntityFact.cs:877`)
  — creating an entity on one client only corrupts RNG sync *and* ids.
...
**Mod rules that follow:** mechanics randomness only from serializable streams inside synced execution;
...
## Desync detection (and what happens after)
...
- **Sliding-window strategy** (`SlidingWindowDesyncDetectionStrategy`): first mismatch fires the "potential
  desync" handler list, which is **EMPTY in release** — sub-2-second desyncs are completely silent. Only
...
- **There is no automatic recovery.** The simulation never stops or resyncs. The only resync path is manual:
  ...
  have to drive; this project deliberately does not — see the no-auto-resync doctrine in `MOD-PLAN.md`.**

## Session lifecycle & save transfer
...
- Disconnect: any real disconnect → `StopPlaying` (leave room, back to lobby/menu); **no `ReconnectAndRejoin`
  ...
  only at area-load ends; dialogs/vendors/chargen/space-combat-exit have no barrier — they rely purely on
  synced commands.
...
- Each client publishes its mod list (Id+Version, from `UserModsData.UsedMods`) as Photon player property
  ...
  invisible entirely. → A sim-affecting mod on one client silently desyncs; **enforcement must come from the
  mod itself** (see the mod-parity doctrine in `MOD-PLAN.md`, and `../ROADMAP-0.9.md` item 1).
- OwlcatModification-template mods (ours) *are* listed in `UsedMods` — identity = manifest UniqueName+Version.
- **39 settings sync** (all difficulty, autopause, TB pacing, gore) via a byte-index protocol
  ...
  the END — the wire format is the array index).
- Console cheats are lockstep-synced `RunCheatCommandGameCommand`s (`CheatGameCommandSystem.cs:47-66`) — a
  ready-made co-op-safe transport for mod actions. Caveat: a cheat registered on only one client executes
...
## Useful built-in QA/testing tools (no mod needed)

- `net_desync_default` cheat → per-tick desync detection (fires on FIRST mismatching tick) — ideal while
...
- `net_state` → dump full HashableState JSON. QA `NetworkingOverlay` shows sync status & skipped ticks.
