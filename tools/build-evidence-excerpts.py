...
to the repository root. The raw captures are not tracked in this repository because they contain other
players' Windows usernames and Steam IDs); they are retained by the author and released only in
redacted form. The script requires the raw archive and exits if a referenced capture is unavailable.
"""
...
A("# Evidence Excerpts")
A("")
A("This document contains small, verbatim extracts from selected two-sided captures. It provides")
A("reviewable evidence without publishing the multi-GB raw logs. Every quoted line is checked against")
A("the hashed source file at generation time. The generator (`tools/build-evidence-excerpts.py`)")
...
A("delivered with its own SHA-256. The quoted lines contain no identifiers, so each quote remains")
A("verifiable against either copy.")
A("")
...
A("---")
A("")
A("## E-DLG: Dialogue UI RNG (capture 0.8.8, confirmed)")
A("")
A("| File | SHA-256 |")
A("|---|---|")
...
A("```")
A("")
A("The host's ring for the same episode contains **no `DialogSystem` entries**:")
A("")
A("```")
...
A("```")
A("")
A("One-sided `DialogSystem` movement at view time supports the C16 diagnosis. The post-fix")
A("validation record is separate (see `../EVIDENCE-MATRIX.md`).")
A("")
A("---")
A("")
A("## E-WEX: Weather combat-exit (capture 0.8.17-SECOND, open investigation C19)")
A("")
A("| File | SHA-256 |")
A("|---|---|")
...
A("---")
A("")
A("## E-TRP: Trap/pause command lifecycle (capture 0.8.xx, supports C20)")
A("")
A("| File | SHA-256 |")
A("|---|---|")
...
A("")
A("Whole-file counts of the residual-command exception (recountable with grep):")
A("")
A("```")
...
A("")
A("The diagnostic records a paused window, a visible unit, a missing IK graph, and an NRE after the")
A("hashed simulation orientation write. The same `(tick, unit, seq)` key appears on both peers,")
A("allowing direct record comparison:")
A("")
A("```")
...
A("```")
A("")
A("Hashes for the earlier 0.8.19 capture of the same behavior:")
A("`%s` (PEER-A), `%s` (PEER-B)." % (sha['PA19'], sha['PB19']))
A("")
A("---")
A("")
A("## E-BRK: Augmentation-screen player-bucket fork (capture 0.8.23, supports C21)")
A("")
A("| File | SHA-256 |")
A("|---|---|")
...
A("identified one-sided write (mechanism chain in `../EVIDENCE-MATRIX.md` and the C21 catalog entry;")
